...
namespace Pathfinding.Examples {
	...
	public class TurnBasedManager : Singleton<TurnBasedManager> {
        ...
        private void Update()
        {
            ...
            if (state == State.SelectTarget) HandleButtonUnderRay(ray);
        ...
        private T GetByRay<T>(Ray2D ray) where T : class
        {
            ...
            var mousePos2D = new Vector2(mousePosition.x, mousePosition.y);
            var hit = Physics2D.Raycast(mousePos2D, Vector2.zero, Mathf.Infinity, layerMask);
        ...
        {
            foreach (var go in possibleMoves) Destroy(go);
            possibleMoves.Clear();
        }
